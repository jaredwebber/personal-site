...
function setBackgroundColour() {
  if (tileColours == GREYSCALE_COLOURS) {
    loadingBackground.style.backgroundColor =
      BACKGROUND_COLOURS["GREYSCALE_COLOURS"];
  ...
  for (var x = 0; x < rows; x++) {
    for (var y = 0; y < cols; y++) {
      ...
      if (!grid[x][y].isAlive && neighbours == REVIVE) {
        ...
        grid[x][y].isAlive &&
        (neighbours <= MIN_NEIGHBOURS || neighbours >= MAX_NEIGHBOURS)
      ) {
